...
#sitka的数据集
#filename = 'test/data/sitka_weather_2018_simple.csv'

#death valley的数据集
filename = 'test/data/death_valley_2018_full.csv'
...
with open(filename) as f:
    ...
    #从文件获得每日降水量和日期
    rainfall_datas, dates = [], []
    for row in reader:
        current_date = datetime.strptime(row[2], '%Y-%m-%d')
        rainfall_data = float(row[3])
        dates.append(current_date)
        rainfall_datas.append(rainfall_data)
    ...
    fig, ax =plt.subplots()
    ax.plot(dates, rainfall_datas, c='red', alpha=0.5)
    #设置图形格式
